...
class GF(nn.Module):
    ...
    def forward(self, src, mask, captions, encode_and_save=True, memory_cache=None, query_type=None, glance=None):
        ...
        if encode_and_save:
            ...
            # print("###############################################################################\n\n\nhs[-1]: ", hs[-1].shape, hs.shape)
            
            if 'qa' in query_type:  # get qa query at focusing stage
                print("get qa query at focusing stage")
            ...
            if 'event' in query_type:  # get memory query at glancing stage
                print("get memory query at glancing stage")
                ...
                print("END --> hs[-1], hs: ", hs[-1].shape, hs.shape)

                print("\n\n\n\n\n")
                print(hs[-1][0, :, :])
                print("\n\n\n\n\n")
                print(hs[-1][1, :, :])
                # assert (hs[-1][0, :, :] == hs[-1][1, :, :]).all(), "not same"
                # assert (hs[-1][1, :, :] == hs[-1][2, :, :]).all(), "not same"
                # assert (hs[-1][2, :, :] == hs[-1][3, :, :]).all(), "not same"
