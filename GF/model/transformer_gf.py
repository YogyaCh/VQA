    ...
    def forward(
        ...
        if encode_and_save:
            ...
            if glance:
                ...
                print("type(img_memory): ", type(img_memory))
                print("img_memory.shape: ", img_memory.shape)

                for key in memory_cache :
                    try : 
                        print(key, " --> ", memory_cache[key].shape)
                    except :
                        print(key) 
                
                # img_memory_np = img_memory.numpy()

                # # Plot each frame
                # fig, axs = plt.subplots(nrows=8, ncols=10, figsize=(20, 16))

                # for i in range(8):
                #     for j in range(10):
                #         frame_index = i * 10 + j
                #         axs[i, j].imshow(img_memory_np[frame_index], cmap='gray')
                #         axs[i, j].axis('off')
                #         axs[i, j].set_title(f'Frame {frame_index}')
...
class TransformerEncoder(nn.Module):
    ...
    def forward(
        ...
        for layer in self.layers:
            print("inside layer in TransformerEncoder")
            output = layer(output, src_mask=mask, src_key_padding_mask=src_key_padding_mask, pos=pos)

        if self.norm is not None:
            print("self.norm is not None in TransformerEncoder")
